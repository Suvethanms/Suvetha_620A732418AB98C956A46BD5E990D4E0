...
class BankAccount:
    ...
    def withdraw(self, amount):
        if 0 < amount <= self.__account_balance:
            self.__account_balance -= amount
            print(f"Withdraw {amount} units. New balance: {self.__account_balance}")
        else:
            print("Withdraw amount must be greater than zero and less than or equal to the account balance.")
...
if __name__ == "__main__":
    account = BankAccount("8650321573", "karunya", 5000)

    account.display_balance()  # Should display the initial balance

    account.deposit(800)  # Depositing 500 units
